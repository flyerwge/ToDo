<template>
  <div class="status-bar" @click="$store.commit('clearAll')">
    ...
    <div class="text">
      ...
      <p class="text-info">
        已打卡 <span class="clock-in-num">{{ todayClockIn }}</span> 项
      </p>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    todoPercentage() {
      return (
        (this.todayCompleted / this.todayListsAll.length).toFixed(2) * 100 || 0
      );
    },
    // 待办描边参数
    ...
    // 今日打卡
    todayClockIn() {
      return this.$store.getters.visibleClockIn.filter(
    ...
    clockInPercentage() {
      return (
        (this.todayClockIn / this.$store.state.clockInAll.length).toFixed(2) *
          100 || 0
      );
    },
    // 打卡描边参数
